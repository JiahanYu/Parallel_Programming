/* compile:
 * mpicc MS_MPI_dynamic.c -o MS_MPI_dynamic -L /opt/X11/lib -l X11
 * execute:
 * $mpiexec -np <numOfProcesses> ./MS_MPI_dynamic <anynumber> <xmin> <xmax> <ymin> <ymax> <XResn> <YResn> <enable/disable>
 * e.g. $mpiexec -np 4 ./MS_MPI_dynamic 4 -2 2 -2 2 400 400 enable
 */
...
int main(int argc, char *argv[])
{
    ...
    if(rank==0){
	    ...
	    FILE *log_file_p;
	    char file_name[100];
	    sprintf(file_name, "MPI_dynamic_%d_processes_execution_time.txt", size);
